...
import org.apereo.openequella.integration.blackboard.buildingblock.TokenGenerator;
...
import org.apereo.openequella.integration.blackboard.common.propbag.PropBagMin.PropBagMinThoroughIterator;
import org.apereo.openequella.web.remoting.soap.SoapService51;
...
public class WrappedUser {
	...
	public String getToken() {
		try {
			...
			return TokenGenerator.createSecureToken(getUsername(), config.getSecretId(), config.getSecret(), getSessionId());
		} catch (IOException e) {
			BbLogger.instance().logError("Error generating token", e);
	...
	public CourseMembership getCourseMembership(CourseMembershipDbLoader membershipLoader, Id courseId) throws Exception {
	...
	private void setContext(HttpServletRequest request) {
		...
		try {
			...
			try {
				...
				BbLogger.instance().logError("Error setting context", ex);
			}
		}
	...
	public void clearContext() {
		try {
			...
			BbLogger.instance().logError("Couldn't release context for user " + username + " " + getSessionId(), e);
		}
	}
	...
	public User getUser() {
		try {
			...
			BbLogger.instance().logError("Error in getUser()", e);
			throw Throwables.propagate(e);
		}
	}
	...
	private UserDbLoader getUserLoader() {
		if (userDbLoader == null) {
			try {
				this.userDbLoader = (UserDbLoader) BbContext.instance().getPersistenceManager().getLoader(UserDbLoader.TYPE);
