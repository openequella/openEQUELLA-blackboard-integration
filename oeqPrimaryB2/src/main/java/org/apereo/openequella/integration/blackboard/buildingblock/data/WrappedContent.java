package org.apereo.openequella.integration.blackboard.buildingblock.data;

import static org.apereo.openequella.integration.blackboard.common.BbUtil.CONTENT_ID;
...
public class WrappedContent implements Comparable<WrappedContent> {
	 ...
	 * 
	 * @param request
	 * @param bbEvaluateURLs
	 *          Use for modify.jsp and modify_proc.jsp. modify.jsp *used* to replace
	 *          the BB placeholders. Don't know what's happened there.
	 */
	...
	public void startSelectionSession(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// else
		// {
		final StringBuilder resourceHref = new StringBuilder(
				PathUtils.urlPath(BbUtil.getBlockRelativePath(), "AddContentBody"));
		// Foward our params
		...
		if (!Strings.isNullOrEmpty(queryString)) {
			...
			if (!queryString.contains(CONTENT_ID)) {
				resourceHref.append("&" + CONTENT_ID + "=").append(URLEncoder.encode(getId().toExternalString(), "UTF-8"));
			}
		}
	...
	/**
	 * Note: does not use Blackboard's sepecial placeholders for courseId, contentId
	 * 
	 * @return
	 */
	...
	public void persist(HttpServletRequest request) throws Exception {
		BbLogger.instance().logTrace("persist(request)");
		final ClassLoader oldLoader = Thread.currentThread().getContextClassLoader();
		try {
			Thread.currentThread().setContextClassLoader(getClass().getClassLoader());
			boolean isNew = false;
			if (content == null) {
				BbLogger.instance().logTrace("content shouldn't be null here...");
				isNew = true;
			...
			if (!displayAfter) {
				BbLogger.instance().logTrace("set start date to null");
				content.setStartDate(null);
			}
			if (!displayUntil) {
				BbLogger.instance().logTrace("set end date to null");
			...
			BbLogger.instance().logTrace("EQUELLA URL is " + equellaUrl);
			BbLogger.instance().logTrace("Calling persistContent");
	...
	public void remove(HttpServletRequest request) throws Exception {
		BbLogger.instance().logDebug("remove(request)");
		...
		try {
			...
			BbLogger.instance().logTrace("EQUELLA URL is " + equellaUrl);
			BbLogger.instance().logTrace("Calling removeContent");
	...
	public void modify(HttpServletRequest request) throws Exception {
		BbLogger.instance().logTrace("modify(request)");
		try {
			...
			BbLogger.instance().logTrace("views=" + views);
			BbLogger.instance().logTrace("described=" + described);
			BbLogger.instance().logTrace("startDateString=" + startDateString);
			BbLogger.instance().logTrace("endDateString=" + endDateString);
			BbLogger.instance().logTrace("displayAfterString=" + displayAfterString);
			...
			BbLogger.instance().logError("Error modifying content", e);
			throw Throwables.propagate(e);
		}
	}
	...
	public synchronized Content getContent() {
		if (content == null) {
			BbLogger.instance().logTrace("content is null, loading via ID=" + getId().toExternalString());
			try {
				content = ContentUtil.instance().loadContent(getId());
				BbLogger.instance().logTrace("content type=" + content.getClass().getCanonicalName());
			} catch (final Exception e) {
				BbLogger.instance().logError("Error getting content", e);
				throw Throwables.propagate(e);
			}
		}
		return content;
		...
		public String getValue(String key) {
			...
			if (value == null && itemInfo != null) {
				...
				if (ContentUtil.FIELD_URL.equals(key)) {
					return getLaunchUrl(PathUtils.urlPath("integ/gen", itemInfo.getItemKey().getUuid(),
