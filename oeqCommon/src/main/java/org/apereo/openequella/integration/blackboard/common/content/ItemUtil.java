...
public class ItemUtil {
  ...
  private static final String HTML_TEMPLATE = "<div class=\"equella-link contextItemDetailsHeaders\"><a {5} href=\"{3}?content_id=@X@content.pk_string@X@&course_id=@X@course.pk_string@X@\">"
      + "<img alt=\"{2}\" src=\"{0}/{1}\" style=\"margin-right:10px\">{4}" + "</a></div>";
  // @formatter:on
  ...
  public static ItemInfo getItemInfo(Content content, Course course, Id folderId, String equellaUrl) {
    ...
    final ItemInfo info = new ItemInfo(equellaUrl, extendedData.getValue(ContentUtil.FIELD_UUID),
        ...
        course.getId().toExternalString(), folderId.toExternalString(), extendedData.getValue(ContentUtil.FIELD_URL));
    info.setAvailable(content.getIsAvailable());
    info.setDescription(extendedData.getValue(ContentUtil.FIELD_DESCRIPTION));
    // Note: we are using ModifiedDate for CreatedDate now. CreatedDate was
    // deprecated and could go away any day now.
