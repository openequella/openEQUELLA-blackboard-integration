...
public class ContentUtil {
	...
	/**
	 * This is a *relative* URL. I.e. it doesn't include the institution portion.
	 */
	...
	public Content addContent(Course course, Id folderId, String uuid, int version, String url, String title,
		...
		try {
			...
			BbLogger.instance().logError("Error saving new content", e);
	...
	public Content persistContent(Content content, Course course, String equellaUrl, boolean isNew) {
		try {
			...
			final FormattedText text = new FormattedText(
					ItemUtil.getHtml(equellaUrl, itemKey.getPage(), itemInfo.getAttachmentName(), itemInfo.getMimeType(),
							content.getTitle(), itemInfo.getDescription(), content.getLaunchInNewWindow()),
					FormattedText.Type.HTML);
			content.setBody(text);

			BbLogger.instance().logTrace("persisting content for url " + itemKey.getPage());
			getContentPersister().persist(content);
			BbLogger.instance().logTrace("content success!");
			...
			BbLogger.instance().logError("Error persisting content", e);
			throw Throwables.propagate(e);
		}
	}
	...
	public Content loadContent(Id contentId) {
		try {
			...
			BbLogger.instance().logError("Error loading content", e);
			throw Throwables.propagate(e);
		}
	}
	...
	private void ensureGradableItem(Content content, boolean isNewContent, boolean modifyExisting) throws Exception {
		...
		if (isNewContent) {
			...
			if (gitem == null) {
				...
				BbLogger.instance().logTrace("gradable item already exists");
				foundExisting = true;
			}
			gradableItem = gitem;
		}
		if (modifyExisting || !foundExisting) {
			...
			try {
				...
				BbLogger.instance().logError("Error saving gradableItem", e);
				throw Throwables.propagate(e);
			}
			BbLogger.instance().logTrace("gradable item success!");
		} else {
			BbLogger.instance().logTrace("not modifying existing gradable item");
		}
	}
	...
	private void register(Content content, Course course, ItemInfo itemInfo, String equellaUrl, boolean isNew) {
		try {
			...
			BbLogger.instance().logDebug("Registering content " + itemKey + " which is " + (isNew ? "new" : "NOT new"));

			final Date createdDate = (content.getCreatedDate() == null ? new Date() : content.getCreatedDate().getTime());
			final Date modifiedDate = (content.getModifiedDate() == null ? createdDate : content.getModifiedDate().getTime());
			...
			BbLogger.instance().logError("An error occurred while registering content", t);
		}
	}
	...
	public void removeContent(Content content, Course course, String equellaUrl, String token) {
		try {
			...
			BbLogger.instance().logError("Error removing content", e);
			throw Throwables.propagate(e);
		}
	}
		...
		try {
			...
			if (gradableItem != null) {
				try {
					...
					BbLogger.instance().logError("Error removing gradableItem", e);
					throw Throwables.propagate(e);
				}
			}
		} catch (Exception e) {
	...
	private void unregister(ItemKey itemKey) {
		try {
			...
			if (reg.isRegistered()) {
				BbLogger.instance().logDebug("Unregistering content " + itemKey + "");
				RegistrationUtil.unrecordItem(reg.getId(), reg.getContentId());
			} else {
				BbLogger.instance().logDebug("Content " + itemKey + " is not registered");
			}
		} catch (Exception t) {
			BbLogger.instance().logError("An error occurred while un-registering content", t);
		}
	}
	...
	private void deactivate(ItemInfo itemInfo, String equellaUrl, String token) {
		...
		try {
			...
			BbLogger.instance().logError("An error occurred while deactivating content", e);
			throw Throwables.propagate(e);
		} finally {
			if (method != null) {
	...
	private ContentDbLoader getContentLoader() {
		if (contentLoader == null) {
			...
			try {
				contentLoader = (ContentDbLoader) BbContext.instance().getPersistenceManager().getLoader(ContentDbLoader.TYPE);
			} catch (final Exception e) {
				BbLogger.instance().logError("Error creating content loader", e);
				throw Throwables.propagate(e);
			}
		}
	...
	private ContentDbPersister getContentPersister() {
		if (contentPersister == null) {
			BbLogger.instance().logTrace("Content persister not yet initialised");
			try {
				contentPersister = (ContentDbPersister) BbContext.instance().getPersistenceManager()
						.getPersister(ContentDbPersister.TYPE);
			} catch (final PersistenceException e) {
				BbLogger.instance().logError("Error creating content persister", e);
				throw Throwables.propagate(e);
			}
		}
