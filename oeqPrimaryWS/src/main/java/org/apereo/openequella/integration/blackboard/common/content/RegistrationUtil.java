...
public abstract class RegistrationUtil
{
		...
		String bbTitle, String bbDesc, String attachmentName, Date dateAdded, Date dateModifed, String courseName)
	{
		...
		final String schema = SqlUtil.schema();
		...
		if( added )
		{
			...
			else if( t == SqlUtil.DbType.SQL_SERVER )
			{
				sql = insert(COL_DATE_CREATED, COL_DATE_MODIFIED, COL_INSTITUTION_URL, COL_UUID, COL_VERSION,
					...
					COL_BLACKBOARD_DESC, COL_PAGE, COL_CONTENT_ID, COL_COURSE_NAME, COL_ATTACHMENT_NAME)
						.append(" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			}
			else if( t == SqlUtil.DbType.POSTGRES )
			{
				sql = insert(COL_DATABASE_ID, COL_DATE_CREATED, COL_DATE_MODIFIED, COL_INSTITUTION_URL, COL_UUID,
					COL_VERSION, COL_COURSE_ID, COL_FOLDER_ID, COL_AVAILABLE, COL_COURSE_AVAILABLE,
					COL_BLACKBOARD_TITLE, COL_BLACKBOARD_DESC, COL_PAGE, COL_CONTENT_ID, COL_COURSE_NAME,
					COL_ATTACHMENT_NAME)
						.append(" VALUES (nextval('equellaseq'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			}
		...
		else
		{
			...
			else
			{
				...
				sql = update(COL_DATE_MODIFIED, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE,
					COL_BLACKBOARD_DESC, COL_PAGE, COL_COURSE_NAME, COL_ATTACHMENT_NAME)
						.append("WHERE INSTURL=? AND UUID=? AND VERSION=? AND COURSEID=? AND FOLDERID=?");
	...
	public static boolean updateDateAccessed(String contentId, Date dateAccessed)
	{
		...
		final List<Integer> updateCount = SqlUtil.runSql(
			update(COL_DATE_ACCESSED).append("WHERE CONTENTID=?").toString(), null, accessedTimestamp, contentId);
		return updateCount.get(0) > 0;
	}
			...
			public List<ItemInfo> getResults(ResultSet results) throws SQLException
			{
				...
				while( results.next() )
				{
					...
					final ItemInfo r = new ItemInfo(institutionUrl, itemUuid, results.getInt(COL_VERSION), contentId,
						results.getString(COL_COURSE_ID), results.getString(COL_FOLDER_ID),
	...
	private static void makeDatabase()
	{
		...
		if( t == SqlUtil.DbType.ORACLE )
		{
			...
			//@formatter:off
			final String sql = "CREATE TABLE equellacontent (" 
				+ "ID NUMBER NOT NULL, "
		...
		else if( t == SqlUtil.DbType.SQL_SERVER )
		{
			...
			//@formatter:off
			final String sql = "CREATE TABLE dbo.equellacontent (" 
		...
		{
			//BbUtil.debug("Postgres DB detected");

			try
			{
				SqlUtil.runSql("CREATE SEQUENCE equellaseq START WITH 1 INCREMENT BY 1 NO MAXVALUE", null);
			}
			catch( Exception e )
			{
				// Ignore, it may already exist
			}

			//@formatter:off
			final String sql = "CREATE TABLE equellacontent (" 
				+ "ID int NOT NULL PRIMARY KEY, " 
				+ "INSTURL varchar(255) NOT NULL, "
				+ "BBTITLE varchar(1024) NOT NULL, "
				+ "PAGE varchar(1024) NULL, " //nullable for item summaries
				+ "DTCREATED timestamp without time zone NOT NULL, "
				+ "DTMODIFIED timestamp without time zone NOT NULL, " 
	...
	private static void modifyDatabaseAddContentAndCourseName()
	{
		...
		{
			// N/A.  There won't be any Postgres DBs this old
		}
	}
