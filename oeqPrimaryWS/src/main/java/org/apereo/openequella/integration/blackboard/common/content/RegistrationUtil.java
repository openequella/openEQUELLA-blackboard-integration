...
import static org.apereo.openequella.integration.blackboard.common.SqlUtil.delete;
import static org.apereo.openequella.integration.blackboard.common.SqlUtil.insert;
import static org.apereo.openequella.integration.blackboard.common.SqlUtil.select;
import static org.apereo.openequella.integration.blackboard.common.SqlUtil.update;
...
import org.apereo.openequella.integration.blackboard.common.SqlUtil.OptionalParam;
...
public abstract class RegistrationUtil {
	// until proven otherwise. The databaseVersion is currently 5 (as of after 6.3
	// GA)
	...
	public static void recordItem(ItemKey itemKey, boolean added, boolean available, boolean courseAvailable,
		...
		BbLogger.instance()
				.logTrace("recordItem(" + itemKey + ", " + added + ", " + available + ", " + courseAvailable + ", " + bbTitle
						+ ", " + bbDesc + ", " + attachmentName + ", " + dateAdded + ", " + dateModifed + ",  " + courseName + ")");
		ensureDatabase();
		...
		final java.sql.Timestamp modifiedTimestamp = new java.sql.Timestamp(
				dateModifed == null ? now : dateModifed.getTime());

		if (added) {
			if (oracle) {
				sql = insert(COL_DATABASE_ID, COL_DATE_CREATED, COL_DATE_MODIFIED, COL_INSTITUTION_URL, COL_UUID, COL_VERSION,
						COL_COURSE_ID, COL_FOLDER_ID, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE,
						COL_BLACKBOARD_DESC, COL_PAGE, COL_CONTENT_ID, COL_COURSE_NAME, COL_ATTACHMENT_NAME).append(" VALUES (")
								.append(schema).append("equellaseq.nextval, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,?, ?, ?, ?)");
			} else {
				sql = insert(COL_DATE_CREATED, COL_DATE_MODIFIED, COL_INSTITUTION_URL, COL_UUID, COL_VERSION, COL_COURSE_ID,
						COL_FOLDER_ID, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE, COL_BLACKBOARD_DESC, COL_PAGE,
						COL_CONTENT_ID, COL_COURSE_NAME, COL_ATTACHMENT_NAME)
								.append(" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			}
			...
			if (!Strings.isNullOrEmpty(contentId)) {
				BbLogger.instance().logDebug("updating recorded usage using contentId");

				sql = update(COL_DATE_MODIFIED, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE, COL_BLACKBOARD_DESC,
						COL_PAGE, COL_COURSE_NAME, COL_ATTACHMENT_NAME).append("WHERE CONTENTID=?");
				List<Integer> updateCount = SqlUtil.runSql(sql.toString(), null, modifiedTimestamp, available, courseAvailable,
						bbTitle, bbDesc, itemKey.getPage(), courseName, attachmentName, contentId);

				int updates = updateCount.get(0);
				if (updates > 1) {
					// There have instances of dupes. Remove them all. This
					// should be a rare occurrence
					BbLogger.instance().logError("** duplicate recordings of " + contentId + " **", null);

					// remove dupes and add a brand new entry
					unrecordItem(0, contentId);
					recordItem(itemKey, true, available, courseAvailable, bbTitle, bbDesc, attachmentName, dateAdded, dateModifed,
							courseName);
				} else if (updates == 0) {
					BbLogger.instance().logDebug("content ID not logged... updating the existing one and record the content ID");

					sql = update(COL_DATE_MODIFIED, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE,
							COL_BLACKBOARD_DESC, COL_PAGE, COL_COURSE_NAME, COL_CONTENT_ID, COL_ATTACHMENT_NAME)
									.append("WHERE INSTURL=? AND UUID=? AND VERSION=? AND COURSEID=? AND FOLDERID=?");
					updateCount = SqlUtil.runSql(sql.toString(), null, modifiedTimestamp, available, courseAvailable, bbTitle,
							trim(bbDesc, 511), itemKey.getPage(), courseName, itemKey.getContentId(), attachmentName,
							itemKey.getInstitutionUrl(), itemKey.getUuid(), itemKey.getVersion(), itemKey.getCourseId(),
							itemKey.getFolderId());
					checkSingleUpdate(updateCount);
				}
			} else {
				BbLogger.instance().logDebug("updating recorded usage using old field matching");

				sql = update(COL_DATE_MODIFIED, COL_AVAILABLE, COL_COURSE_AVAILABLE, COL_BLACKBOARD_TITLE, COL_BLACKBOARD_DESC,
						COL_PAGE, COL_COURSE_NAME, COL_ATTACHMENT_NAME)
								.append("WHERE INSTURL=? AND UUID=? AND VERSION=? AND COURSEID=? AND FOLDERID=?");
	...
	private static boolean checkSingleUpdate(List<Integer> updateCount) {
		...
		if (updates != 1) {
			...
			BbLogger.instance().logError("** Updated " + updates + "  rows when only one should have been updated **", null);
			return false;
		}
		return true;
	}
	...
	private static boolean unrecordItem(int usageId) {
		BbLogger.instance().logDebug("unrecordItem(" + usageId + ")");
		ensureDatabase();
	...
	public static boolean updateRecordedTitle(String contentId, String title, String description) {
		BbLogger.instance().logDebug("updateRecordedTitle(" + contentId + ", " + title + ", " + description + ")");
		ensureDatabase();
	...
	public static boolean updateRecordedLocation(String contentId, String courseId, String folderId) {
		BbLogger.instance().logDebug("updateRecordedLocation(" + contentId + ", " + courseId + ", " + folderId + ")");
		ensureDatabase();
	...
	public static boolean updateDateAccessed(String contentId, Date dateAccessed) {
		BbLogger.instance().logDebug("updateDateAccessed(" + contentId + ", " + dateAccessed + ")");
		ensureDatabase();

		final Timestamp accessedTimestamp = new Timestamp(dateAccessed.getTime());
		final List<Integer> updateCount = SqlUtil.runSql(update(COL_DATE_ACCESSED).append("WHERE CONTENTID=?").toString(),
				null, accessedTimestamp, contentId);
		return updateCount.get(0) > 0;
	}
	...
	public static List<ItemInfo> findUsages(final String institutionUrl, final String itemUuid, final int itemVersion,
		...
		BbLogger.instance().logDebug("findUsages(" + institutionUrl + ", " + itemUuid + ", " + itemVersion + ", "
		...
		}, institutionUrl, itemUuid, new OptionalParam<Integer>(itemVersion, !allVersions),
				new OptionalParam<Boolean>(available, available), new OptionalParam<Boolean>(available, available));
		return usages;
	}
	...
	public static List<ItemInfo> findAllUsages(final String institutionUrl, final String query, final String courseId,
		...
		BbLogger.instance().logDebug("findAllUsages(" + institutionUrl + ", " + query + ", " + courseId + ", " + folderId
		...
		final List<ItemInfo> usages = SqlUtil.runSql(sql.toString(), new ResultProcessor<ItemInfo>() {
			...
			public List<ItemInfo> getResults(ResultSet results) throws SQLException {
				final List<ItemInfo> u = new ArrayList<ItemInfo>();
				while (results.next()) {
					final ItemInfo r = new ItemInfo(institutionUrl, results.getString(COL_UUID), results.getInt(COL_VERSION),
							results.getString(COL_CONTENT_ID), results.getString(COL_COURSE_ID), results.getString(COL_FOLDER_ID),
		...
		}, institutionUrl, new OptionalParam<String>(likeQuery, hasQuery), new OptionalParam<String>(courseId, hasCourseId),
				new OptionalParam<String>(folderId, hasFolderId), new OptionalParam<Boolean>(available, available),
	...
	public static List<ItemKey> findEquellaContentByCourse(final String institutionUrl, final String courseId,
			boolean available) {
		BbLogger.instance()
				.logDebug("findEquellaContentByCourse(" + institutionUrl + ", " + courseId + ", " + available + ")");
		ensureDatabase();

		// ID, UUID, VERSION, FOLDERID, ATTACHMENT_NAME, CONTENTID
		final StringBuilder sql = select(COL_DATABASE_ID, COL_UUID, COL_VERSION, COL_FOLDER_ID, COL_PAGE, COL_CONTENT_ID)
				.append("WHERE INSTURL=? AND COURSEID=?");
		if (available) {
			sql.append(" AND AVAILABLE=? AND COURSEAVAILABLE=?");
		}
		...
		}, institutionUrl, courseId, new OptionalParam<Boolean>(available, available),
				new OptionalParam<Boolean>(available, available));
		return usages;
	}

	public static int cleanupBadContent(final String institutionUrl) {
		BbLogger.instance().logDebug("cleanupBadContent(" + institutionUrl + ")");
		ensureDatabase();
	...
	public static ContentRegisteredResponse contentIsRegistered(ItemKey itemKey) {
		BbLogger.instance().logDebug("contentIsRegistered(" + itemKey + ")");
		ensureDatabase();
		...
		if (itemKey.getContentId() != null) {
			...
			res = SqlUtil.runSql(sql.toString(), resultProcessor, itemKey.getInstitutionUrl(), itemKey.getUuid(),
					itemKey.getVersion(), itemKey.getCourseId(), itemKey.getFolderId(),
					new OptionalParam<String>(page, !Strings.isNullOrEmpty(page)));
		}
	...
	private static void makeDatabase() {
		...
		if (getDatabase().isOracle()) {
			BbLogger.instance().logDebug("Oracle DB detected");

			SqlUtil.runSql("CREATE SEQUENCE equellaseq START WITH 1 INCREMENT BY 1 NOMAXVALUE", null);

			// @formatter:off
			sql = "CREATE TABLE equellacontent (" + "ID NUMBER NOT NULL, " + "INSTURL NVARCHAR2(255) NOT NULL, "
					+ "DTCREATED DATE DEFAULT SYSDATE, " + "DTMODIFIED DATE DEFAULT SYSDATE, "
					+ "BBTITLE NVARCHAR2(1024) NOT NULL," + "PAGE NVARCHAR2(1024)," // nullable for item summaries
					+ "UUID NVARCHAR2(40) NOT NULL, " + "VERSION NUMBER NOT NULL," + "COURSEID NVARCHAR2(32) NOT NULL, "
					+ "FOLDERID NVARCHAR2(32) NOT NULL, " + "AVAILABLE NUMBER NOT NULL," + "COURSEAVAILABLE NUMBER NOT NULL,"
			...
			BbLogger.instance().logDebug("SQL Server DB detected");
			// @formatter:off
			sql = "CREATE TABLE dbo.equellacontent (" + "ID int NOT NULL PRIMARY KEY IDENTITY, "
					+ "INSTURL nvarchar(255) NOT NULL, " + "BBTITLE nvarchar(1024) NOT NULL, " + "PAGE nvarchar(1024) NULL, " // nullable
																																																										// for
																																																										// item
																																																										// summaries
					+ "DTCREATED datetime NOT NULL, " + "DTMODIFIED datetime NOT NULL, " + "UUID nvarchar(40) NOT NULL, "
					+ "VERSION int NOT NULL," + "COURSEID nvarchar(32) NOT NULL, " + "FOLDERID nvarchar(32) NOT NULL, "
					+ "AVAILABLE bit NOT NULL," + "COURSEAVAILABLE bit NOT NULL" + ")";
			// @formatter:on
			SqlUtil.runSql(sql, null);
			...
			// cannot be UNIQUE since same item could be added to same folder
			// (in theory)
			SqlUtil.runSql("CREATE INDEX eqcontent_all ON dbo.equellacontent(INSTURL, UUID, VERSION, COURSEID, FOLDERID)",
					null);
	...
	private static void modifyDatabaseAddContentAndCourseName() {
		if (getDatabase().isOracle()) {
			...
			// @formatter:off
			final String sql = "ALTER TABLE equellacontent ADD (" + "CONTENTID NVARCHAR2(32) NULL, "
			...
			// @formatter:off
			final String sql = "ALTER TABLE dbo.equellacontent ADD " + "CONTENTID nvarchar(32) NULL, "
