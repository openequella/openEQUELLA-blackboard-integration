 ...
 * last entry in the zip file, otherwise you are screwed. It is also not allowed
 * to get too big, otherwise you run into another Blackboard bug.
 * 
 * @author Aaron
 */
...
public class ZEquellaWebserviceImpl implements EquellaWebservice {
  ...
  public Course[] listCoursesForUser(@WebParam(mode = WebParam.Mode.IN, name = "username") String username,
    ...
    try {
      ...
      if (modifiableOnly) {
        coursesWithCreate = SecurityUtil.getCourseIdsWithEntitlement(session.getUserId(),
  ...
  public AddItemResult addItemToCourse(@WebParam(mode = WebParam.Mode.IN, name = "username") String username,
    ...
    try {
      ...
      // final String itemUrl = PathUtils.urlPath("integ/gen", itemUuid,
      // Integer.toString(itemVersion)) + url;
  ...
  public SearchResult findUsages(@WebParam(mode = WebParam.Mode.IN, name = "serverUrl") String serverUrl,
    ...
    WebServiceUtil.debug(
        "Found " + usages.size() + " usages of " + itemUuid + "/" + itemVersion + " for institutionUrl " + serverUrl);
  ...
  /**
   * findAllUsages does not do unrecording etc because it needs to be as fast as
   * possible
   */
  @Override
  ...
  public boolean editContent(@WebParam(mode = WebParam.Mode.IN, name = "contentId") String contentId,
    ...
    try {
      ...
      if (ContentUtil.instance().isLegacy(content)) {
        ...
        html = ItemUtil.getHtml(institutionUrl, extendedData.getValue("url"), extendedData.getValue("attachmentName"),
            extendedData.getValue("mimeType"), title, description, content.getLaunchInNewWindow());
      }
