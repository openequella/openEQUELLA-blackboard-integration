      ...
      if (suspectStart > 0) {
        ...
        if (wrappedReadSize >= 0) {
          for (int i = 0; i < wrappedReadSize; ++i) {
            ...
            if ((next < 0x20 && next != 0x9 && next != 0xA && next != 0xD) || (next > 0xD7FF && next < 0xE000)
                || (next > 0xFFFD && next < 0x10000) || next > 0x10FFFF) {
              // discard
              didDiscard = true;
            } else {
              // are we suspicious?
  ...
  protected int writeSuspect(final char[] output, final int position, final int maxLength) {
    ...
    if (suspect.length < len + suspectStart || output.length < len + position || len < 0) {
      throw new ArrayIndexOutOfBoundsException(
          "suspect buffer size: " + suspect.length + ", output buffer size: " + output.length + ", suspect start: "
              + suspectStart + ", position: " + position + ", maxLength: " + maxLength);
    }
  ...
  protected boolean isDodgy() {
    ...
    if (suspect[1] == '#') {
      if (suspectLength > 2) {
        ...
        } else if (c3 == '0' || c3 == '1' || c3 == '2' || c3 == '3' || c3 == '4' || c3 == '5' || c3 == '6' || c3 == '7'
            || c3 == '8' || c3 == '9') {
