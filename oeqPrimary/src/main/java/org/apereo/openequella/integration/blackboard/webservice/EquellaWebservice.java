...
public interface EquellaWebservice {
   ...
   * 
   * @return The version of the webservice. This will only be incremented once the
   *         webservice is changed and there exists a released version of the old
   *         webservice in the wild.
   * @since 1
   */
  ...
  /**
   * @param username
   *          unused. Please use ContextWS.emulateUser before invoking this
   *          method.
   ...
   * @param itemVersion
   * @param url
   *          Appears to be of the form ?attachment.uuid=blah
   * @param title
   * @param description
   * @param xml
   * @param serverUrl
   * @param attachmentUuid
   *          UUID of the attachment. Specify null for the item summary
  ...
  /**
   * @param serverUrl
   *          The institution URL. Should be the same as configured in the
   *          Building Block
   * @param itemUuid
   *          The uuid of the item to get usage for.
   * @param itemVersion
   *          The version of the item to get usage for. Unused if allVersions
   *          specified
   * @param available
   *          If true, will only check courses that are available
   * @param allVersions
   *          Find all versions of said item
   ...
   *          Building Block
   * @param query
   *          A freetext query to match external content name. ie. the name of the
   *          resource in Blackboard
   * @param courseId
   *          restrict to this course
   * @param folderId
  ...
  /**
   * Scans for EQUELLA content and updates the EQUELLA DB table for quick access
   * 
   * @param institutionUrl
   * @param available
   *          Only check available courses and available registered content
   * @return Always true
   * @since 1
   */
  ...
  /**
   * Get the course code e.g. EQ101 for the supplied Blackboard internal course ID
   * 
   * @param courseId
